# EDA on IMDB Movie Dataset 🎬

## Project Overview
Exploratory Data Analysis on a dataset of 3000 movies to help a movie producer 
decide what type of movies to produce and which actors to cast.

## Business Problem
A rookie movie producer needs data-driven recommendations on:
- What type of movies to produce
- Which actors to cast

## Questions Answered
1. Which movie made the highest profit? Who were its producer and director?
2. Which language has the highest average ROI (Return on Investment)?
3. What are the unique genres in this dataset?
4. Top 3 producers with the highest average ROI
5. Which actor has acted in the most movies? (genre & profit deep dive)

## Tools Used
- Python
- Pandas
- Numpy
- Matplotlib
- Google Colab

## Dataset
IMDB Movies Dataset (3000 movies)
